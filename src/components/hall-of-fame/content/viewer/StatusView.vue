      ...
      <div class="status-element-group">
        <template v-for="key in StatusKeys" :key="key">
          ...
          <div class="status-element" :class="key">
            <div class="element-label" v-t="`glossary.status.${key}`" />
...
import { getStatusGradeIcon } from "/@/util/resources/image";
import { HallOfFameDTO } from "/@/db/hall-of-fame";
import { useI18n } from "vue-i18n";
import { StatusKeys } from "/@/data/status";
  ...
  > .status-view-wrapper {
    ...
    > .status-element-group {
      ...
      > .status-element {
        ...
        > .element-label {
          @apply text-[0.75rem] text-[#fefefe] tracking-wider;
        }
        > .element-value {
          ...
          > .text {
            @apply flex-grow text-[0.95rem] text-right;
          }
        }
      }
    }
